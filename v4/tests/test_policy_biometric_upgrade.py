...
Ported from v3/tests/test_policy_biometric_upgrade.py; env fixture replaced
by Config injection (defaults already match the v3 fixture's env).
...
def test_upgrade_disabled_by_config():
    # v3 intent: MIB_UPGRADE_WAIVED_ON_BIOMETRIC=0 disables the upgrade.
    adj, _, tag = apply_policy(
        {}, "NEEDS_REVIEW", 0.70, "R_A1_non_dip_waived_TO_REVIEW",
        _bundle(text=CLEAN_SLIP_TEXT),
        config=Config(upgrade_waived_on_biometric=False),
    )
    assert adj == "NEEDS_REVIEW"
    assert tag == "R_A1_non_dip_waived_TO_REVIEW"
