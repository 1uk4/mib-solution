...
V1's `_reject_placeholder` handles this at extraction time by returning
"" when a matched value equals a bracketed placeholder. This sanitizer
does the same job at L3, one layer earlier — content that reaches L4
is already free of these markers, so all downstream extractors and
signal emitters don't need to know about them.
