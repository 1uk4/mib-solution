...
class TestGateOff:
    def test_reocr_gate_off(self, monkeypatch):
        """When MIB_CHAR_WHITELIST_REOCR=0, _norm_and_repair skips repair.

        Verifies that the gate env var controls the signals._REOCR_ENABLED
        flag and that _norm_and_repair returns the plain normalized value
        without calling repair(), regardless of the field type.
        """
        import importlib
        import v3.signals as signals_module

        # Patch the module-level flag to simulate gate-off.
        monkeypatch.setenv("MIB_CHAR_WHITELIST_REOCR", "0")
        # Temporarily override the module-level boolean (already evaluated at
        # import time) so this test doesn't depend on import order.
        monkeypatch.setattr(signals_module, "_REOCR_ENABLED", False)
        ...
        result = signals_module._norm_and_repair("sponsor_id", "SPN-6O99", src)
